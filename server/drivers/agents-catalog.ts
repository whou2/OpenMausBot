...
import { optionsCardEnabledForBot, OPTIONS_CARD_LIMITS } from "../../shared/options-card.ts";
...
const toolDefinitions = (externalRuntime: boolean) => [
  {
    ...
    description:
      "Show the person a native card with 2-6 choices in this conversation. The card is passive: a click returns the selected words as a reply and never authorizes or performs an external action. Use it for review and draft-selection steps, then wait for the person's response.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
      properties: {
...
export function availableTools(profile: CatalogProfile) {
  ...
  const BOT_SCOPED_TOOLS = optionsCardEnabledForBot(profile.botId)
    ? TOOLS
